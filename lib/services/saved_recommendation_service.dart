...
class SavedRecommendationService {
  ...
  Future<void> saveRecommendationSession({
    ...
    if (user == null) {
      throw Exception('Please sign in before saving a recommendation.');
    }

    final topRecommendations = recommendations.take(3).toList();

    if (topRecommendations.isEmpty) {
      throw Exception('No recommendation is available to save.');
    }
  ...
  Future<void> deleteRecommendationSession(String id) async {
    final user = _supabase.auth.currentUser;
  ...
  Map<String, dynamic> _recommendationSnapshot({
    ...
    return {
      ...
      'property_types': property.normalizedPropertyTypes,
      ...
      'advantages': List<String>.from(recommendation.reasons),

      'cautions': List<String>.from(recommendation.cautions),

      'scheme': property.scheme,

      'project_status': property.projectStatus,
      ...
      'unit_options': property.unitOptions
  ...
  List<Map<String, dynamic>> _resolveAppliedWeights({
    required UserPreferences preferences,
    required List<double> appliedWeights,
  }) {
    final labels = preferences.goal == PropertyGoal.ownStay
        ? const ['Safety', 'Education facilities', 'Transportation']
        : const [
    ...
    if (appliedWeights.length >= 3) {
      weights = appliedWeights.take(3).toList();
    } else {
      final priorities = preferences.goal == PropertyGoal.ownStay
          ? [
      ...
      weights = _normalisePriorities(priorities);
    }
